...
import `in`.porter.cfms.api.service.recon.usecases.FetchReconTasksService
...
class FetchReconTasksHttpService @Inject
constructor(
    private val fetchReconTasksService: FetchReconTasksService,
    private val listReconRequestMapper: ListReconRequestMapper
) : Traceable {
    private val logger = LoggerFactory.getLogger(FetchReconTasksHttpService::class.java)
    ...
    suspend fun invoke(
        ...
        try {
            ...
            val reconResponse: FetchReconResponse = fetchReconTasksService.invoke(page, size)
            ...
            call.respond(
                ...
                mapOf("error" to listOf(mapOf("message" to "Failed to retrieve recon entries", "details" to e.message)))
            )
        }
    }
}